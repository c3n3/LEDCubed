...
public:
    ...
    static uint16_t directionalCubeArray(const uint8_t& firstCord, const uint8_t& secondCord, const uint8_t& thirdCord, directions direction, bool setLED = false, uint16_t color = 0);
    ...
    // Switch from a normal coordinate to directional
    static void switchToDirectionalCoord(ui8& nonRelativeX, ui8& nonRelativeY, ui8& nonRelativeZ, directions direction);

    // delete all of a color on the entire cube
    static void eradicate(uint16_t color);

    //TODO: delete this thing if safe
    static void setRandomLED(uint8_t leds, directions direction, uint16_t color, bool threeD = false);
